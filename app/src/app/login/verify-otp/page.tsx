...
const OTPValidate: React.FC = () => {
    ...
    const phone_number = parsedls.phone_number
        ...
        if (parsedls.otp_session && parsedls.login_type) {
            axios.get("https://befakeapi.netlify.app/api/verify-otp", {
                headers: {
                    ...
                    otp_session: parsedls.otp_session
                }
            }).then(response => {
                ...
                toast.error("Erreur lors de la verification de l'OTP")
                if (typeof window !== "undefined") {
                    localStorage.removeItem('OTPSession');
                    router.replace("/login/phone-number")
                }
    ...
    return (
        ...
        <div className='h-[100vh] flex flex-col justify-between'>
            ...
            <div className="flex items-center justify-center flex-col h-[90vh]">
                <h1 className="text-xl font-bold -mt-24">Verifiez votre numéro de téléphone</h1>
                <input
                    type="text"
                    value={OTPcode}
                    min={0}
                    ...
                    className="bg-black border-white border-opacity-50 rounded-none text-left tracking-[6px] w-24 focus:outline-none mt-8"
                    id="otp"
                    inputMode='numeric'
                    maxLength={6}
                />
                ...
                <p className="text-stone-400 text-sm text-center mt-6">
                    Code de vérification envoyé au <b>{phone_number}</b>
                </p>
                <button
                className={`px-6 py-2 bg-white text-black font-medium rounded-lg transition-all mt-4 focus:ring-2 focus:ring-white/20 outline-none hover:bg-white/10  ${loading ? "text-white" : "text-black"} ${OTPcode.length !== 6 || loading ? "disabled:opacity-50 disabled:cursor-not-allowed": ""}`}
            ...
            <p className='flex justify-center mb-5 text-sm'>Ce site n'est en aucun cas affilié à BeReal SAS.</p>
        </div>
        </>
    )
};
