...
const Login: React.FC = () => {
    ...
    const checkSend = (otp: {status: number; data: any;}) => {
        setLoading(false)
        if (otp.status == 201) {
            setOTPsession(otp.data.otpSession)
            setView(false)
        }
        else {
            setError(true)
    ...
    const sendOTP = () => {
        ...
        fetch("https://berealapi.fly.dev/login/send-code", requestOptions)
        .then(response => response.text())
        .then(result => checkSend(JSON.parse(result)))
        .catch(() => setError(true));
    }

    const OTPredirect = (otp: {status: number; data: any;}) => {
        setLoading(false)
        if (otp.status == 201) {
    ...
    const validateOTP = () => {
        ...
        fetch("https://berealapi.fly.dev/login/verify", requestOptions)
        .then(response => response.text())
        .then(result => OTPredirect(JSON.parse(result)))
        .catch(() => setError(true));
    }
    ...
    return (
        ...
        <div className={view ? "block" : "hidden"}>
            <svg width="110" height="45" viewBox="0 0 2000 824" fill="none" xmlns="http://www.w3.org/2000/svg" className='absolute w-full'>
            ...
            <div className="flex items-center justify-center flex-col h-[100vh]">
                ...
                <h1 className={`mt-6 mx-9 text-red-600 ${error ? "block" : "hidden"}`}>Une erreur est survenue, veuillez réessayer ultérieurement.</h1>
            </div>
        </div>
