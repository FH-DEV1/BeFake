        ...
        <div className="flex flex-col">
            <div className='flex flex-row mt-5 mx-5 justify-between items-center'>
                ...
                <div className='flex flex-col justify-center items-center'>
                    ...
                    <p className='opacity-60'>{UTCtoParisTime(post?.takenAt ? post?.takenAt : "1999-12-31T23:00:00.000Z")}{post?.lateInSeconds !== 0 ? ` - ${formatTime(post?.lateInSeconds ? post?.lateInSeconds : 0)}` : ""}</p>
                </div>
                <div className='' onClick={() => router.push(`/profile/${post?.user.id}`)}>
            ...
            <div className='flex items-center w-full justify-center'>
                <div className={`${post?.location ? "block" : "hidden"} mx-2 bg-zinc-800 flex pr-2 mt-5 rounded-full text-white`}>
                    ...
                    >
                        {post?.location?.ReverseGeocode ? `${post?.location?.ReverseGeocode?.City}, ${post?.location?.ReverseGeocode?.CntryName}` : "ouvrir dans maps"}
                    </a>
                </div>
            </div>
