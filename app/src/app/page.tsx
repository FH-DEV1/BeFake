...
const Home: React.FC = () => {
  ...
  const getFeed = (JWToken: string) => {
    ...
    const validateOTP = () => {
        ...
        .then(result => {
          ...
          if (JSON.parse(result).status == 201) {
            if (typeof window !== "undefined") {
              localStorage.setItem('token', JSON.parse(result).data.token);
              getFeed(token == null ? "error" : token)
              setPage("feed")
    ...
    return (
        <>
        <div>
            <svg width="110" height="45" viewBox="0 0 2000 824" fill="none" xmlns="http://www.w3.org/2000/svg" className={`${page == "start" || page == "onePostFeed" || page == "feedError" ? "hidden" : "block"} absolute w-full`}>
