...
import { PersonRounded, NearMe, ScreenRotationAlt, KeyboardBackspaceRounded, AddAPhoto } from '@mui/icons-material';
...
const Home: React.FC = () => {
    ...
    const UTCtoParisTime = (utcTime: string) => {
    ...
    const UTCtoParisDate = (utcTime: string) => {
      ...
      fetch(`/api/feed`, requestOptions)
        ...
        .then(result => {
            ...
            if (parsedResult.friendsPosts) {
              ...
              toast.warning("Erreur lors du chargement des Bereal.1");
              setPage("feedError");
              setLoading(false);
            }
        })
    ...
    const getMyUserInfo = () => {
      ...
      .then(result => {
        if (JSON.parse(result).status == 200) {
          ...
          fetch(`${domain}/friends/mem-feed`, requestOptions)
          .then(response => response.text())
          .then(result => {
            if (JSON.parse(result).status == 200) {
              setnewmemories(JSON.parse(result).data.data)
              setPage("MyProfile")
              setLoading(false)
            }
            else {
    ...
    return (
        ...
        <div>
            ...
            <div className={`${page == "start" || page == "onePostFeed" || page == "MyProfile" ? "hidden" : "block"} fixed w-full z-50 flex justify-between items-center`}>
              <div onClick={() => toast.warn("posting is not available yet")}>
                <AddAPhoto className='h-7 w-7 ml-2'/>
              </div>
              <a href="https://github.com/FH-DEV1/BeFake" className=''>
            ...
            <div className={`${page == "onePostFeed" ? "block" : "hidden"} flex flex-col`}>
              <div className='flex flex-row mt-5 mx-5 justify-between items-center'>
                ...
                <div className='flex flex-col justify-center items-center'>
                  ...
                  <p className='opacity-60'>{UTCtoParisTime(selectedPost.takenAt)}{selectedPost.lateInSeconds !== 0 ? ` - ${formatTime(selectedPost.lateInSeconds)}` : ""}</p>
                </div>
            ...
            <div className={`${page == "MyProfile" ? `block` : "hidden"} flex flex-col items-center`}>
              ...
              <img className='h-36 w-36 rounded-full' src={MyInfo.data.profilePicture.url} alt="profile picture" />
              ...
              <p className='text-md mt-10'>Actif depuis le {UTCtoParisDate(MyInfo.data.createdAt)}</p>
            </div>
