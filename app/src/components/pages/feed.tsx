...
import { FriendPost, Post, ReverseGeocodeData, TagsResponse } from '../Types';
...
const Feed: React.FC = () => {
    const { setPage, prevPage, setPrevPage, scrollPos, setScrollPos, feed, setFeed, setSelectedPost, gridView, setGridView } = usePageState();
    ...
    useEffect(() => {
        // Fetch reverse geocode data for all posts
        const fetchReverseGeocodes = async () => {
            const updatedReverseGeocodes: ReverseGeocodeData = {};
            if (feed.friendsPosts != undefined) {
                for (const post of feed.friendsPosts) {
                    for (const userPost of post.posts) {
                        if (userPost.location) {
                            const reverseGeocode = await fetchReverseGeocode(userPost.location.latitude, userPost.location.longitude);
                            updatedReverseGeocodes[userPost.id] = reverseGeocode;
                        }
                    }
                }
    ...
    return (
        <>
            <div className="fixed w-full z-50 flex justify-between items-center">
                ...
                <div onClick={() => {setGridView(false); setPage("MyProfile")}}>
            ...
            <div className="pt-11 pb-11">
                <div className={`${feed?.userPosts ? isScrolled ? "block" : "hidden" : "hidden"} z-50`}>
                    <div className='flex text-white justify-center mt-2 fixed w-full z-50'>
                        ...
                        <p className="ml-2 opacity-50" onClick={() => {setGridView(false); setPage("FOFfeed")}}>Amis d'Amis</p>
                    </div>
                </div>

                <div className={`flex flex-col-reverse ${feed?.userPosts ? "mt-8" : "mt-0"} ${gridView ? "hidden" : ""}`}>
                                    ...
                                    <div className='flex flex-col' key={`${userPost.id}_${imageIndex}`}>
                                        <div className='flex mb-1.5'>
                                            ...
                                            <div className='flex-col ml-2'>
                                                ...
                                                >
                                                    {userPost.location ? `${reverseGeocodes[userPost.id] !== undefined ? reverseGeocodes[userPost.id] : "ouvrir dans maps"} • ` : ""}
                ...
                <div className={`flex flex-wrap justify-around mt-16 ${gridView ? "" : "hidden"}`}>
                    {feed && feed.friendsPosts?.map((userPost: FriendPost, imageIndex: number) => (
                        <div className='flex flex-col mb-2' key={`${userPost.posts[0].id}_${imageIndex}`} onClick={() => {
                            setSelectedPost({
                                from: "feed",
                                user: userPost.user,
                                post: userPost.posts[0],
                                realMojis: userPost.posts[0].realMojis,
                            })
                            setScrollPos(window.scrollY)
                            setPage("SelectedPost")
                        }}>
                            <div className='relative'>
                                <img
                                    ...
                                    src={selectedImages[`${userPost.posts[0].id}_${imageIndex}`] ? userPost.posts[0].secondary.url : userPost.posts[0].primary.url}
                                    alt={`Image ${imageIndex}`} />
                                <img
                                    ...
                                    src={selectedImages[`${userPost.posts[0].id}_${imageIndex}`] ? userPost.posts[0].primary.url : userPost.posts[0].secondary.url}
                                    alt={`Image ${imageIndex}`} />
