          ...
          if (JSON.parse(result).status == 200) {
            ...
            .then(result => {
                if (JSON.parse(result).status == 200) {
                    ...
                    setmemories(JSON.parse(result).data.data)
                    setPage("MyProfile")
                    setLoading(false)
                }
                else {
                    toast.warning("Erreur lors du chargement des memories.")
                    setLoading(false)
                }
            })
            .catch(() => {toast.error("Erreur lors du chargement des memories.");setLoading(false)});
          }
          else {
