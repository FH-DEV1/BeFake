...
const Login: React.FC = () => {
    ...
    const sendOTP = () => {
        ...
        fetch(`/api/login/send-code`, requestOptions)
        .then(response => response.text())
        .then(result => {
            setLoading(false)
            if (JSON.parse(result).status !== "error") {
                if (typeof window !== "undefined") {
                    localStorage.setItem('OTPSession', JSON.parse(result).sessionInfo);
                    router.replace("/login/verify-otp")
                    toast.success("OTP envoyé avec succès!")
                }
            } else {
                toast.error(`Erreur : ${JSON.parse(result).error.error.message}`)
            }
        })
        .catch((error) => toast.error("Erreur lors de l'envoie de l'OTP."));
    }
    return (
        <>
            ...
            <div className="flex items-center justify-center flex-col h-[90vh]">
                ...
                <PhoneInput
                    ...
                    id="phone" />
                <button onClick={sendOTP} className={`${loading ? "hidden" : "block"} mt-6 py-1 px-3 rounded-md bg-zinc-900 border-2 border-zinc-700`}>Suivant</button>
                <div className={`${loading ? "block" : "hidden"} mt-[26px] h-8 w-8 animate-spin rounded-full border-2 border-solid border-current border-r-transparent align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite]`} role="status">
